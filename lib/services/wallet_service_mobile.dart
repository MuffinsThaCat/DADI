...
class WalletServiceMobile extends WalletServiceInterface {
  ...
  static const String _stakedAmountKey = 'staked_amount_mobile';
  static const String _stakingRewardsKey = 'staking_rewards_mobile';
  static const String _stakingStatusKey = 'staking_status_mobile';
  static const String _stakingStartTimeKey = 'staking_start_time_mobile';
  static const String _stakingLockupPeriodKey = 'staking_lockup_period_mobile';
  ...
  Future<double> get pChainStakedAmount async {
    ...
    try {
      final stakedAmountStr = await _secureStorage.read(key: _stakedAmountKey);
      return stakedAmountStr != null ? double.parse(stakedAmountStr) : 0.0;
    } catch (e) {
  ...
  Future<bool> get isCreatorStakingActive async {
    ...
    try {
      final statusStr = await _secureStorage.read(key: _stakingStatusKey);
  ...
  Future<double> get stakingRewards async {
    ...
    try {
      final rewardsStr = await _secureStorage.read(key: _stakingRewardsKey);
      final rewards = rewardsStr != null ? double.parse(rewardsStr) : 0.0;
      
      // In a real implementation, you would calculate real-time rewards
      // based on staking duration and amount
      final stakingStartTimeStr = await _secureStorage.read(key: _stakingStartTimeKey);
      if (stakingStartTimeStr != null) {
        final stakingStartTime = int.parse(stakingStartTimeStr);
  ...
  Future<String> stakeForCreator({
    ...
    try {
      ...
      final currentBalanceEth = await balance;
      if (currentBalanceEth < amount) {
      ...
      final txId = 'p-' + DateTime.now().millisecondsSinceEpoch.toString() + 
                 '-' + (1000000 + (DateTime.now().millisecondsSinceEpoch % 1000000)).toString();
      ...
      await _secureStorage.write(key: _stakedAmountKey, value: newStakedAmount.toString());
      await _secureStorage.write(key: _stakingStatusKey, value: 'true');
      
      // Set staking start time if this is initial stake
      if (currentStakedAmount <= 0) {
        await _secureStorage.write(
          key: _stakingStartTimeKey, 
      ...
      if (lockupPeriodDays != null) {
        await _secureStorage.write(
          key: _stakingLockupPeriodKey,
          value: lockupPeriodDays.toString()
        );
      }
  ...
  Future<String> unstakeFromCreator({
    ...
    try {
      ...
      // Check lockup period
      final lockupPeriodStr = await _secureStorage.read(key: _stakingLockupPeriodKey);
      final stakingStartTimeStr = await _secureStorage.read(key: _stakingStartTimeKey);
      
      if (lockupPeriodStr != null && stakingStartTimeStr != null) {
        final lockupPeriod = int.parse(lockupPeriodStr);
        final stakingStartTime = int.parse(stakingStartTimeStr);
        final currentTime = DateTime.now().millisecondsSinceEpoch;
        final stakedDays = (currentTime - stakingStartTime) / (1000 * 60 * 60 * 24);
      ...
      if (newStakedAmount <= 0) {
        await _secureStorage.write(key: _stakingStatusKey, value: 'false');
        await _secureStorage.write(key: _stakingStartTimeKey, value: '0');
        await _secureStorage.write(key: _stakingLockupPeriodKey, value: '0');
      }
  ...
  Future<String> claimStakingRewards() async {
    ...
    try {
      ...
      // Reset rewards
      await _secureStorage.write(key: _stakingRewardsKey, value: '0.0');
  ...
  Future<Map<String, dynamic>> getCreatorProfile() async {
    ...
    try {
      ...
      int stakingStartTime = 0;
      int lockupPeriod = 0;
      
      if (stakingStartTimeStr != null) {
        stakingStartTime = int.parse(stakingStartTimeStr);
      }
      
      if (lockupPeriodStr != null) {
        lockupPeriod = int.parse(lockupPeriodStr);
      }
      ...
      if (stakingStartTime > 0 && lockupPeriod > 0) {
        final currentTime = DateTime.now().millisecondsSinceEpoch;
        final stakedDays = (currentTime - stakingStartTime) / (1000 * 60 * 60 * 24);
        remainingLockupDays = (lockupPeriod - stakedDays.floor()) > 0 
            ? (lockupPeriod - stakedDays.floor()) 
            : 0;
      }
